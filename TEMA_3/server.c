...
void lock_for_reading(pthread_mutex_t *counter_mutex, pthread_mutex_t *writeMutex, int *counter)
{
    pthread_mutex_lock(counter_mutex);
    (*counter) = (*counter)+1;
    printf("Counter_LOCK: %d\n", *counter);
    if ((*counter) == 1)
    {
        pthread_mutex_lock(writeMutex);
    }
...
void unlock_for_reading(pthread_mutex_t *counter_mutex, pthread_mutex_t *writeMutex, int *counter)
{
    pthread_mutex_lock(counter_mutex);
    printf("Counter_UNLOCK: %d\n", *counter);
    *(counter) = (*counter) - 1;
    printf("Counter_UNLOCK: %d\n", *counter);
    if ((*counter) == 0)
    {
...
void write_in_logFile(char *opcode, char *fileName, char *word)
{
    char buffer[1024];
    time_t current_time = time(NULL);                // Get current time
    struct tm *time_info = localtime(&current_time); // Convert to local time

    strftime(buffer, sizeof(buffer), "%x, %X, ", time_info); // Format date and time
    ...
    write(logFileDes, buffer, strlen(buffer));
...
void ls_send(char *command, int *sock)
{
    ...
    write_in_logFile(LIST, NULL, NULL);
}
...
void get_funct(char *command, int *client)
{
    ...
    printf("COUNTER_AFTERLOCK: %d\n",filesInUse.counter[index]);
    ...
    printf("COUNTER_AFTERUNLOCK: %d\n",filesInUse.counter[index]);

    write_in_logFile(GET, fileName, NULL);
    free(fileName);
}
...
void deleteFunct(char *command, int *client)
{
    ...
    write_in_logFile(DELETE, fileName, NULL);
    free(fileName);
}
...
void putFunct(char *command, int *client)
{
    ...
    write_in_logFile(PUT, fileName, NULL);
...
void updateFunct(char *command, int *client)
{
    ...
    write_in_logFile(UPDATE, fileName, NULL);
...
void createLogFile()
{
    logFileDes = open("log.txt", O_WRONLY | O_CREAT, 0644);
