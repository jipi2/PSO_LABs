...
static int get_ascii(unsigned int scancode)
{
	...
	if (scancode >= 0x1e && scancode <= 0x26)
		return *(row3 + scancode - 0x1e);s
	if (scancode >= 0x2c && scancode <= 0x32)
		return *(row4 + scancode - 0x2c);
	if(scancode == 0xf)
		return '\t';
	/* if (scancode >= 0x1a && scancode <= 0x1d)
		return *(row7 + scancode - 0x2c); */
	if(scancode == 0x2b)
		return '\\';
	if(scancode == 0x35)
		return '/';		
			 ...
			 size_t size, loff_t *offset)
{
	...
	spin_lock_irqsave(&data->s_lock, flags);
	reset_buffer(data);
	spin_unlock_irqrestore(&data->s_lock, flags);
...
static int kbd_init(void)
{
    ...
    resource = request_region(I8042_DATA_REG+1,1,MODULE_NAME);
    if(resource == NULL)
    {
        err = -EBUSY;
        goto out_unregister ;
    ...
    resource = request_region(I8042_STATUS_REG+1,1,MODULE_NAME);
    if(resource == NULL)
    {
        err = -EBUSY;
        goto out_unregister;
    ...
    if(err != 0)
    {
        printk("Apelul request_irq a dat fail\n");
