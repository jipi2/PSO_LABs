...
int main(int argc, char **argv)
{
	...
	mem_piece = fmap.size/NUM_THREADS;
	...
	for(int i=0;i<NUM_THREADS;i++)
	{
		//pthread_join(th_id[i],(void **)&results[i]);    // cum se paseaza parametrul ????
		pthread_join(th_id[i],(void **)&aux_res);
		total+=*aux_res;
	}
	...
	/* for (i = 0; i < NUM_THREADS; ++i)
		free(results[i]);					//comentat pt ca nu ma folosesc de result[];
 */
